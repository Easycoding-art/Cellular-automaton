...
class EffectMaker :
    def __init__(self, file_name) :
        ...
        for i in range(s) :
            for j in range(h) :
                for n in range(len(self.__colors)) :
                    if ((pix[i, j][0] >= self.__colors[n][0] - 50 and pix[i, j][0] <= self.__colors[n][0] + 50) and
                        (pix[i, j][1] >= self.__colors[n][1] - 50 and pix[i, j][1] <= self.__colors[n][1] + 50) and
                        (pix[i, j][2] >= self.__colors[n][2] - 50 and pix[i, j][2] <= self.__colors[n][2] + 50)) :
                        self.__matrix[i, j] = n
                        break
        os.remove("frame.jpg")
